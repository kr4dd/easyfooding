...
public class AccesoModelo {
  ...
  public ArrayList<ListaPedidos> historial(String usuario) {
    ...
    Cursor c =
        singletonInstance
            .getDb()
            .rawQuery("SELECT * FROM pedidos WHERE nombre_usuario = ?", new String[] {usuario});
  ...
  public ArrayList<LineaPedidos> getLineaPedidos(int numPedido) {
    ...
    Cursor c =
        singletonInstance
            .getDb()
            .rawQuery("SELECT * FROM linea_pedidos WHERE num_pedido = ?", new String[] {toQuery});
    ...
    while (c.moveToNext()) {
      ...
      LineaPedidos l =
          new LineaPedidos(num_linea, num_pedidio, codigo_comida, cantidad, precio_linea);
      toret.add(l);
    }
  ...
  public int getMaxIdPedido() {
    ...
    Cursor c =
        singletonInstance.getDb().rawQuery("SELECT MAX(num_pedido)  AS max_id FROM pedidos", null);
    String toret = "";
    if (c.moveToNext()) {
      toret = c.getString(0);
    }
